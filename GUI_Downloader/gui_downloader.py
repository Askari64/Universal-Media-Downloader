...
class App(ctk.CTk):
    ...
    def display_quality_options(self, is_playlist, info):
        ...
        if not is_playlist and info:
            ...
            # This is a simplified size estimation for the GUI
            best_video_size = sum(f.get('filesize') or f.get('filesize_approx') or 0 for f in formats if f.get('height') == 1080)
            std_video_size = sum(f.get('filesize') or f.get('filesize_approx') or 0 for f in formats if f.get('height') == 720)
            low_video_size = sum(f.get('filesize') or f.get('filesize_approx') or 0 for f in formats if f.get('height') == 480)
            
            audio_streams = sorted([f for f in formats if f.get('vcodec') == 'none' and f.get('abr')], key=lambda x: x['abr'], reverse=True)
            best_audio_size = audio_streams[0].get('filesize') or audio_streams[0].get('filesize_approx') or 0 if audio_streams else 0
            ...
            sizes = [best_video_size, std_video_size, low_video_size, best_audio_size, std_audio_size, low_audio_size]
        else:
            sizes = [0] * 6 # No sizes for playlists

        for i, ((text, value), size) in enumerate(zip(options_map.items(), sizes)):
            label = f"{text} (~{format_size(size)})" if not is_playlist else text
...
class App(ctk.CTk):
        ...
        try:
            with yt_dlp.YoutubeDL({'quiet': True, 'extract_flat': True}) as ydl:
                ...
                if 'entries' in info and info.get('playlist_count'):
                    ...
                    self.after(0, self.display_quality_options, False)
        except Exception as e:
            error_message = f"Error: {str(e).splitlines()[-1]}"
            if 'DRM' in str(e):
                error_message = "Error: This content is DRM protected."
    ...
    def handle_playlist_or_single(self, url):
        ...
        self.clear_options()
        if choice == "playlist":
            self.display_quality_options(True)
        else:
            self.display_quality_options(False)

    def display_quality_options(self, is_playlist):
        """Displays a standardized list of quality options."""
        if is_playlist:
            self.status_label.configure(text="Status: Choose a quality for the entire playlist.")
        else:
        ...
        self.download_choice.set(options[0][1]) # Default to best video
        self.download_button.configure(state="normal")
        self.enable_fetch_button()
